...
public class ChatbotController {
    ...
    private void addDraftPanel(PostDraft draft) {
        ...
        addPost.setOnAction(e -> {
            try {
                if (UserSession.getInstance().getUser().getId() <= 0) {
                ...
                moderationService.validatePost(UserSession.getInstance().getUser().getId(), title, content);
                ...
                try { pointsService.addPoints(UserSession.getInstance().getUser().getId(), 10); } catch (Exception ignored) {}
                try { activityService.log(UserSession.getInstance().getUser().getId(), "CREATE_POST", "postId=" + newId + " (chatbot)"); } catch (Exception ignored) {}
