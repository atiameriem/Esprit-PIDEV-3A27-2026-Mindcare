...
public class ForumController {
    ...
    private void refreshPosts() {
        ...
        try {
            List<Post> posts = postService.findPosts(isMyTab(), UserSession.getInstance().getUser().getId(), search, sort);
    ...
    private void refreshProfileSidebar() {
        ...
        // Name
        Label full = new Label(UserSession.getInstance().getUser().getFullname() == null ? "" : UserSession.getInstance().getUser().getFullname());
        full.getStyleClass().add("profileName");
        recommendedContainer.getChildren().add(full);

        try {
            int uid = UserSession.getInstance().getUser().getId();
        ...
        ContextMenu postMenu = UiFactory.buildIconMenu(
                ...
                () -> {
                    try {
                        ...
                        activityService.log(UserSession.getInstance().getUser().getId(), "DELETE_POST", "postId=" + p.getId());
...
HBox actions = new HBox(10);
        ...
        likeBtn.setOnAction(e -> {
            try {
                boolean likedNow = likeService.togglePostLike(p.getId(), UserSession.getInstance().getUser().getId());

                // Notifications + activity
                if (likedNow && p.getIdUsers() != UserSession.getInstance().getUser().getId()) {
                    notificationService.create(
                            p.getIdUsers(),
                            "LIKE_POST",
                            UserSession.getInstance().getUser().getFullname() + " a aimé ton post : " + p.getTitle(),
                            "POST",
                            Math.toIntExact(p.getId())
                    );
                    refreshNotifBadge();
                }
                activityService.log(UserSession.getInstance().getUser().getId(), likedNow ? "LIKE_POST" : "UNLIKE_POST", "postId=" + p.getId());
                refreshPosts();
            } catch (SQLException ex) {
                ex.printStackTrace();
                showAlert(Alert.AlertType.ERROR, "Erreur", ex.getMessage());
        ...
        publish.setOnAction(ev -> {
            ...
            try {
                ...
                moderationService.validateComment(UserSession.getInstance().getUser().getId(), txt.trim(), 3);

                commentService.addComment(p.getId(), UserSession.getInstance().getUser().getId(), txt.trim());

                // Scoring + activity
                try { pointsService.addPoints(UserSession.getInstance().getUser().getId(), 2); } catch (SQLException e2) { System.err.println("[POINTS] " + e2.getMessage()); }
                activityService.log(UserSession.getInstance().getUser().getId(), "CREATE_COMMENT", "postId=" + p.getId());

                // Notification au propriétaire du post
                if (p.getIdUsers() != UserSession.getInstance().getUser().getId()) {
                    notificationService.create(
                            p.getIdUsers(),
                            "COMMENT_POST",
                            UserSession.getInstance().getUser().getFullname() + " a commenté ton post : " + p.getTitle(),
                            "POST",
                            Math.toIntExact(p.getId())
                    );
                    refreshNotifBadge();
...
private void openEditPostDialog(Post p) {
    ...
    private void loadCommentsInto(VBox list, long postId) {
        ...
        try {
            ...
            for (Commentaire c : comments) {
                ...
                like.setOnAction(e -> {
                    try {
                        boolean likedNow = likeService.toggleCommentLike(c.getId(), UserSession.getInstance().getUser().getId());
                        activityService.log(UserSession.getInstance().getUser().getId(), likedNow ? "LIKE_COMMENT" : "UNLIKE_COMMENT", "commentId=" + c.getId());
                        loadCommentsInto(list, postId);
                    } catch (SQLException ex) {
                        ex.printStackTrace();
                        showAlert(Alert.AlertType.ERROR, "Erreur", ex.getMessage());
                ...
                ContextMenu cMenu = UiFactory.buildIconMenu(
                        ...
                        () -> {
                            try {
                                ...
                                activityService.log(UserSession.getInstance().getUser().getId(), "DELETE_COMMENT", "commentId=" + c.getId());
                                loadCommentsInto(list, postId);
                                refreshPosts();
                            } catch (SQLException ex) {
                                ex.printStackTrace();
        ...
        dialog.setResultConverter(bt -> {
            if (bt == addType) {
                try {
                    ...
                    Post p = new Post();
                    p.setIdUsers(UserSession.getInstance().getUser().getId());
                    ...
                    // Modération
                    moderationService.validatePost(UserSession.getInstance().getUser().getId(), p.getTitle(), p.getContent());

                    long newId = postService.createPostWithOptionalImage(p, storedPath);

                    // Scoring + activité
                    try { pointsService.addPoints(UserSession.getInstance().getUser().getId(), 10); } catch (SQLException e2) { System.err.println("[POINTS] " + e2.getMessage()); }
                    activityService.log(UserSession.getInstance().getUser().getId(), "CREATE_POST", "postId=" + newId);
    ...
    private void openReportDialogPost(long postId) {
        ...
        dlg.showAndWait().ifPresent(reason -> {
            try {
                reportService.reportPost(postId, UserSession.getInstance().getUser().getId(), reason, null);
                activityService.log(UserSession.getInstance().getUser().getId(), "REPORT_POST", "postId=" + postId + ", reason=" + reason);
    ...
    private void openReportDialogComment(long commentId) {
        ...
        dlg.showAndWait().ifPresent(reason -> {
            try {
                reportService.reportComment(commentId, UserSession.getInstance().getUser().getId(), reason, null);
                activityService.log(UserSession.getInstance().getUser().getId(), "REPORT_COMMENT", "commentId=" + commentId + ", reason=" + reason);
                showAlert(Alert.AlertType.INFORMATION, "OK", "Signalement envoyé.");
            } catch (SQLException ex) {
                ex.printStackTrace();
                showAlert(Alert.AlertType.ERROR, "Erreur", ex.getMessage());
    ...
    private void refreshNotifBadge() {
        ...
        try {
            int unread = notificationService.countUnread(UserSession.getInstance().getUser().getId());
    ...
    private void onNotifClick(ActionEvent event) {
        ...
        try {
            var items = notificationService.listUnread(UserSession.getInstance().getUser().getId(), 15);
            if (items.isEmpty()) {
                ...
                markAll.setOnAction(e -> {
                    try {
                        notificationService.markAllRead(UserSession.getInstance().getUser().getId());
                        refreshNotifBadge();
                    } catch (SQLException ex) {
                        ex.printStackTrace();
                    }
    ...
    private void applyAdminTabVisibility() {
        ...
        System.out.println("[FORUM] admin? " + show + " | sessionRole=" + Permissions.currentRole() + " | id=" + UserSession.getInstance().getUser().getId());
