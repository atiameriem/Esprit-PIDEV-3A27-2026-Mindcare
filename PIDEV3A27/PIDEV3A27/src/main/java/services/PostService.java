...
public class PostService {
    ...
    /**
     * Posts recommandés: top N par score (likes + 2*comments + 0.1*views).
     * Score calculé côté SQL pour éviter de faire plusieurs requêtes.
     */
    public List<Post> findTopScoredPosts(int limit) throws SQLException {
        Connection cnx = cnx();
        String sql =
                "SELECT p.id, p.id_users, u.nom AS authorNom, u.prenom AS authorPrenom, " +
                        "p.title, p.content, p.created_at, p.updated_at, p.views_count, " +
                        ...
                        "((SELECT COUNT(*) FROM post_likes pl WHERE pl.id_post=p.id) + " +
                        "  2*(SELECT COUNT(*) FROM comments c WHERE c.id_post=p.id AND c.status='PUBLISHED') + " +
                        "  0.1*p.views_count) AS score " +
                        "FROM post p " +
    ...
    private Post mapPost(ResultSet rs) throws SQLException {
        ...
        // optionnel: utilisé par "Recommandés"
        try {
    ...
    public int countPostsByUser(int idUser) throws SQLException {
        Connection cnx = cnx();
        String sql = "SELECT COUNT(*) FROM post WHERE id_users = ?";
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ps.setInt(1, idUser);
            try (ResultSet rs = ps.executeQuery()) {
